...
export default function App() {
  ...
  const userCounts = useMemo(() => {
    ...
    users.forEach((user) => {
      if (user.tenantId) counts.set(user.tenantId, (counts.get(user.tenantId) ?? 0) + 1)
    })
    return counts
  }, [users])
  ...
  if (!authorized) {
    return (
      <div className="center-screen stack narrow">
        ...
        <p className="muted">This Firebase account is not registered as a Parthone Super Admin.</p>
  ...
  return (
    <div className="app-shell">
      <aside>
        ...
        <button className="ghost" onClick={() => signOut(auth)}><LogOut size={17} /> Sign out</button>
      </aside>
      ...
      <main className="content">
        ...
        <section className="stats-grid">
          ...
          <article><span>Active Clients</span><strong>{activeClients}</strong></article>
          <article><span>Client Users</span><strong>{clientUsers}</strong></article>
        </section>
